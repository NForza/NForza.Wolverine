...
public class IssueEndpointTests : IntegrationContext
{
    ...
    [Fact]
    public async Task should_build_issue_summary_projection()
    {
        // Create an issue, assign it, then close it
        var originatorId = new UserId();
        var (_, createResult) = await TrackedHttpCall(x =>
        {
            x.Post.Json(new CreateIssue(originatorId, "Projection test", "Testing IssueSummary projection"))
                .ToUrl("/api/issues");
        });
        ...
        // Query the projection via Marten directly using LINQ
        await using var session = Store.QuerySession();
        var summary = await session.Query<IssueSummary>()
            .Where(s => s.Id == created.Id)
            .FirstOrDefaultAsync();
        summary.ShouldNotBeNull();
        summary.Title.ShouldBe("Projection test");
        summary.Status.ShouldBe("Closed");
        summary.AssigneeId.ShouldBe(assigneeId);
        summary.EventCount.ShouldBe(3);

        // Also verify via the HTTP endpoint
        var getResult = await Scenario(x =>
        {
            x.Get.Url($"/api/issues/{created.Id}/summary");
            x.StatusCodeShouldBe(200);
        });

        var httpSummary = getResult.ReadAsJson<IssueSummary>();
        httpSummary.ShouldNotBeNull();
        httpSummary.Title.ShouldBe("Projection test");
        httpSummary.Status.ShouldBe("Closed");
        httpSummary.EventCount.ShouldBe(3);
    }
}
