...
internal static class DateTimeOffsetValueTemplates
{
...
public partial record struct {{Name}}(DateTimeOffset Value) : IDateTimeOffsetValueType, IComparable, IComparable<{{Name}}>, IEquatable<{{Name}}>
...
#endif
{
    ...
    public static bool operator >({{Name}} left, {{Name}} right) => left.CompareTo(right) > 0;
    public static bool operator >=({{Name}} left, {{Name}} right) => left.CompareTo(right) >= 0;
    public static explicit operator DateTimeOffset({{Name}} typedId) => typedId.Value;
    public static explicit operator {{Name}}(DateTimeOffset value) => new(value);
    ...
    {
        if (DateTimeOffset.TryParse(s, System.Globalization.CultureInfo.InvariantCulture, System.Globalization.DateTimeStyles.RoundtripKind, out var value))
        {
