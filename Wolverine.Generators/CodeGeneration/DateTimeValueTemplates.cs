...
internal static class DateTimeValueTemplates
{
...
public partial record struct {{Name}}(DateTime Value) : IDateTimeValueType, IComparable, IComparable<{{Name}}>, IEquatable<{{Name}}>
...
#endif
{
    ...
    public static bool operator >({{Name}} left, {{Name}} right) => left.CompareTo(right) > 0;
    public static bool operator >=({{Name}} left, {{Name}} right) => left.CompareTo(right) >= 0;
    public static explicit operator DateTime({{Name}} typedId) => typedId.Value;
    public static explicit operator {{Name}}(DateTime value) => new(value);
    ...
    {
        if (DateTime.TryParse(s, System.Globalization.CultureInfo.InvariantCulture, System.Globalization.DateTimeStyles.RoundtripKind, out var value))
        {
