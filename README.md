...
Turn a one-line declaration into a full record struct with JSON serialization, `TryParse`, comparison operators, and Marten extension methods:
...
Supported backing types: `Guid` (`[GuidValue]`), `string` (`[StringValue]`), `int` (`[IntValue]`), `double` (`[DoubleValue]`).
...
Runs all 34 tests across 3 test projects (16 generator + 10 IssuesAPI + 8 reporting).
